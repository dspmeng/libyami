    ...
    const SharedPtr<VideoFrame>& dst)
{
    ...
    bool bRotate = m_transform & (VPP_TRANSFORM_ROT_90 | VPP_TRANSFORM_ROT_180 | VPP_TRANSFORM_ROT_270);

    if (bFlip && bRotate) {
        uint32_t width = srcImage->getWidth();
        uint32_t height = srcImage->getHeight();
        if (!m_scratchImage || width != m_scratchImage->getWidth() || height != m_scratchImage->getHeight()) {
        ...
        ERROR("neither flip nor rotate is specified");
        return YAMI_INVALID_PARAM;
    }
